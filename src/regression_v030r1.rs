...
"#;
  ...
  // ── active_key préservée après édition (fix item 0) ──────────────────────

  #[test]
  fn active_key_preservee_apres_application_formulaire() {
    ...
    let original = Service {
      ...
      pending_key: None,
      created_at: NaiveDate::from_ymd_opt(2026, 1, 1).unwrap(),
      last_rotation: Some(NaiveDate::from_ymd_opt(2026, 3, 1).unwrap()),
      deploy_mode: DeployMode::Automatic,
      deployments: vec![],
    };

    let mut form = ServiceFormState::from_service(&original);
    form.name = "Après".into();
    let built = form.build().unwrap();

    // Simuler le merge (comportement de SubmitServiceForm après le fix)
    let mut merged = original.clone();
    merged.name = built.name;
    merged.service_type = built.service_type;
    merged.deploy_mode = built.deploy_mode;
    merged.params.url = built.params.url;
    merged.params.user = built.params.user;
    merged.params.port = built.params.port;
    if built.params.token_ref.is_some() {
      merged.params.token_ref = built.params.token_ref;
    }

    assert_eq!(
      merged.active_key,
      Some(key_id),
      "active_key doit être préservée"
    );
    assert_eq!(
      merged.last_rotation,
      Some(NaiveDate::from_ymd_opt(2026, 3, 1).unwrap())
    );
    assert_eq!(merged.name, "Après");
  }

  // ── token_ref préservé si token non re-saisi ──────────────────────────────

  #[test]
  fn token_ref_preservee_si_token_vide_a_ledition() {
    use crate::app::ServiceFormState;
    use crate::config::model::Service;
    ...
    // Formulaire d'édition — token non re-saisi
    let form = ServiceFormState::from_service(&original);
    assert!(
      form.token_value.is_empty(),
      "token_value ne doit pas être pré-rempli"
    );

    let built = form.build().unwrap();
    // built.params.token_ref == None car token_value est vide

    // Merge : token_ref doit être préservé depuis l'original
    let mut merged = original.clone();
    if built.params.token_ref.is_some() {
      merged.params.token_ref = built.params.token_ref;
    }
    ...
    assert_eq!(
      merged.params.token_ref,
      Some("github_perso".into()),
      "token_ref doit être préservé si le token n'a pas été re-saisi"
    );
  }
